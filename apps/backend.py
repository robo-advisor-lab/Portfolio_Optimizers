...
PRIVATE_KEY = os.getenv('INTEREST_PORTFOLIO_KEY')
ACCOUNT_ADDRESS = os.getenv('INTEREST_PORTFOLIO_ADDRESS')
...
def convert_to_usd(balances, prices):
    ...
    susde_bal_usd = balances['susde'] * prices['SUSDE_Price']
    sdai_bal_usd = balances['sdai'] * prices['SDAI_Price']
    cdai_bal_usd = balances['cdai'] * prices['CDAI_Price']
    return susde_bal_usd, sdai_bal_usd, cdai_bal_usd 
...
def get_balance():
    ...
    try:
        ...
        susde_contract = w3.eth.contract(address=Web3.to_checksum_address(SUSDE_CONTRACT_ADDRESS), abi=erc20_abi)
        cdai_contract = w3.eth.contract(address=Web3.to_checksum_address(CDAI_CONTRACT_ADDRESS), abi=erc20_abi)
        sdai_contract = w3.eth.contract(address=Web3.to_checksum_address(SDAI_CONTRACT_ADDRESS), abi=erc20_abi)

        # Fetch balances
        susde_balance_wei = susde_contract.functions.balanceOf(ACCOUNT_ADDRESS).call()
        cdai_balance_wei = cdai_contract.functions.balanceOf(ACCOUNT_ADDRESS).call()
        sdai_balance_wei = sdai_contract.functions.balanceOf(ACCOUNT_ADDRESS).call()

        # Convert from Wei to token units
        susde_balance = susde_balance_wei / 10**18
        cdai_balance = cdai_balance_wei / 10**18
        sdai_balance = sdai_balance_wei / 10**18
...
def create_app():
    ...
    async def run_model():
        ...
        global today, days_left, prices_df, three_month_tbill, current_risk_free,is_running
        if is_running:
        ...
        try:
            ...
            prices_query = yield_portfolio_prices(start_date)
            print(f'prices_query: {prices_query}')
            #when first starting the price query, if no prices for current hour may need to reindex to + timedelta(hours=1) or find another way to ensure it starts in current hour, even if don't have latest price yet.  or set model start_time to start_time + timedelta(hours=1)
            ...
            prices = {
                    'CDAI_Price': float(prices_df['CDAI_Price'].to_frame('CDAI Price').iloc[-1].values[0]),
                    'SDAI_Price': float(prices_df['SDAI_Price'].to_frame('SDAI Price').iloc[-1].values[0]),
                    'SUSDE_Price': float(prices_df['SUSDE_Price'].to_frame('SUSDE Price').iloc[-1].values[0])
                }
            
            original_holdings = {
                'CDAI': float(original_balances['cdai']),
                'SDAI': float(original_balances['sdai']),
                'SUSDE': float(original_balances['susde'])
            }

            print(f'initial prices for usd conversion: {prices}')
            print(f'initial balances used for usd conversion: {original_balances}')

            susde_bal_usd, sdai_bal_usd, cdai_bal_usd  = convert_to_usd(original_balances, prices)
            initial_portfolio_balance = susde_bal_usd + sdai_bal_usd + cdai_bal_usd

            susde_composition = susde_bal_usd / initial_portfolio_balance
            sdai_composition = sdai_bal_usd / initial_portfolio_balance
            cdai_composition = cdai_bal_usd / initial_portfolio_balance

            comp_dict = {
                "sUSDE comp": susde_composition,
                "sDAI comp": sdai_composition,
                "cDAI comp": cdai_composition,
            ...
            def run_sim():
                ...
                rebalancing_frequency = 36

                model = PPO.load("AI_Models/interest_portfolio")
            ...
            if should_rebalance(current_time, model_actions, rebalancing_frequency):
                ...
                initial_holdings = {
                    'susde': float(initial_balances['susde']),
                    'cdai': float(initial_balances['cdai']),
                    'sdai': float(initial_balances['sdai'])
                }

                print(f'initial holdings {initial_holdings}')

                new_compositions = {
                    "susde": float(model_actions.iloc[-1]["SUSDE_weight"]),
                    "cdai": float(model_actions.iloc[-1]["CDAI_weight"]),
                    "sdai": float(model_actions.iloc[-1]["SDAI_weight"])
                }

                print(f'new compositions: {new_compositions}')

                total_value = sum(initial_holdings[token] * prices[f"{token.upper()}_Price"] for token in initial_holdings)
                target_balances = {token: total_value * new_compositions.get(token, 0) / prices[f"{token.upper()}_Price"] for token in initial_holdings}
                ...
                rebal_info = {
                    ...
                    "SUSDE bal usd": susde_bal_usd,
                    "CDAI bal usd": cdai_bal_usd,
                    "SDAI bal usd": sdai_bal_usd,
        ...
        new_holdings = {
            'CDAI': float(new_balances['cdai']),
            'SDAI': float(new_balances['sdai']),
            'SUSDE': float(new_balances['susde'])
        }

        print(f'initial prices for usd conversion: {prices}')
        print(f'new balances used for usd conversion: {new_balances}')

        susde_bal_usd, sdai_bal_usd, cdai_bal_usd  = convert_to_usd(new_balances, prices)
        new_portfolio_balance = susde_bal_usd + sdai_bal_usd + cdai_bal_usd
        ...
        susde_composition = susde_bal_usd / new_portfolio_balance
        sdai_composition = sdai_bal_usd / new_portfolio_balance
        cdai_composition = cdai_bal_usd / new_portfolio_balance
