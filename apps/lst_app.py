...
def main(model,path,rebalance_frequency,start_date,
    ...
    test_data, train_data, dao_advisor_prices_df = data_processing(api=api,function=lst_portfolio_prices,
                                                                   start_date=start_date,name=model,
                                                                   training_percentage=training_percentage,path=path,
                                                                   data_start_date=data_start_date)
    
    test_data = test_data.ffill().bfill()
    # test_data.set_index('hour',inplace=True)
    ...
    percent_cols = [col for col in dao_advisor_combined_analysis_comp.columns if col.endswith('_Percentage')]

    model_fig3 = visualization_pipeline(
        df=dao_advisor_combined_analysis_comp[percent_cols],
...
if __name__ == "__main__":
    main(seed=20,rebalance_frequency=730,api=False,start_date='2024-06-01 00:00:00',
         model='monthly_lst_model',
         path='data/lst_prices.csv',
